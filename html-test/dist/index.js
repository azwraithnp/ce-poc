...
function shouldUpdateComponent(prevVNode, nextVNode, optimized) {
  ...
  if (false && (prevChildren || nextChildren) && isHmrUpdating) {
    return true;
  }
...
const PublicInstanceProxyHandlers = {
  ...
  set({ _: instance }, key, value) {
    ...
    if (key[0] === "$" && key.slice(1) in instance) {
      ...
      if (false && key in instance.appContext.config.globalProperties) {
...
function applyOptions(instance) {
  ...
  if (dataOptions) {
    if (false && !isFunction(dataOptions)) {
      warn(
...
const normalizeVNodeSlots = (instance, children) => {
  if (false && !isKeepAlive(instance.vnode) && !false) {
    warn(
  ...
  return function createApp(rootComponent, rootProps = null) {
    ...
    const app = (context.app = {
      ...
      component(name, component) {
        ...
        if (false && context.components[name]) {
          warn(
      ...
      directive(name, directive) {
        ...
        if (false && context.directives[name]) {
          warn(
      ...
      mount(rootContainer, isHydrate, isSVG) {
        if (!isMounted) {
          // #5571
          if (false && rootContainer.__vue_app__) {
            warn(
      ...
      provide(key, value) {
        if (false && key in context.provides) {
          warn(
...
function baseCreateRenderer(options, createHydrationFns) {
  ...
  const patch = (
    ...
    isSVG = false,
    slotScopeIds = null,
    optimized = false && isHmrUpdating ? false : !!n2.dynamicChildren
  ) => {
    if (n1 === n2) {
      return;
    }
  ...
  const patchElement = (
    ...
    if (dynamicChildren) {
      ...
      if (false && parentComponent && parentComponent.type.__hmrId) {
  ...
  const mountComponent = (
    ...
    if (false && instance.type.__hmrId) {
    ...
    // i = 2, e1 = 4, e2 = 5
      ...
      for (i = s2; i <= e2; i++) {
        ...
        if (nextChild.key != null) {
          if (false && keyToNewIndexMap.has(nextChild.key)) {
            warn(
...
function traverseStaticChildren(n1, n2, shallow = false) {
  ...
  if (isArray(ch1) && isArray(ch2)) {
    for (let i = 0; i < ch1.length; i++) {
      ...
      if (false && c2.type === Comment && !c2.el) {
        c2.el = c1.el;
      }
    }
  }
...
const Fragment = Symbol(false ? "Fragment" : undefined);
const Text = Symbol(false ? "Text" : undefined);
const Comment = Symbol(false ? "Comment" : undefined);
const Static = Symbol(false ? "Static" : undefined);
...
function _createVNode(
  ...
  const shapeFlag = isString(type)
    ...
    : 0;
  if (false && shapeFlag & 4 /* STATEFUL_COMPONENT */ && isProxy(type)) {
    type = toRaw(type);
    warn(
...
function cloneVNode(vnode, extraProps, mergeRef = false) {
  ...
  const cloned = {
    ...
    children:
      false && patchFlag === -1 /* HOISTED */ && isArray(children)
...
function handleSetupResult(instance, setupResult, isSSR) {
  ...
  } else if (false && setupResult !== undefined) {
    warn(
...
function finishComponentSetup(instance, isSSR, skipOptions) {
  ...
  if (false && !Component.render && instance.render === NOOP && !isSSR) {
    /* istanbul ignore if */
...
function _sfc_render(_ctx, _cache) {
  return openBlock(), createElementBlock("h2", null, "Hello");
}
const SayHello = /*#__PURE__*/ _export_sfc(_sfc_main, [
  ["render", _sfc_render],
]);

const Hello = defineCustomElement(SayHello);

function register(tagName = "say-hello") {
